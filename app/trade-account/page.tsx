...
export default function TradeAccountPage() {
  ...
  useEffect(() => {
    ...
    if (data.deliveryAddress) {
      setDeliveryAddress({ ...emptyAddress, ...data.deliveryAddress });
  ...
  async function saveAddresses(event: FormEvent<HTMLFormElement>) {
    ...
    setError("");
    try {
      await saveTradeAccountData({ billingAddress, deliveryAddress: sameAsBilling ? billingAddress : deliveryAddress });
    ...
    <>
      ...
      <main className="min-h-screen bg-white text-black">
        <div className="mx-auto max-w-4xl px-6 py-20">
          ...
          <p className="mt-4 text-slate-600">Signed in as <strong>{user.email}</strong>. Your saved details will be used when ordering on account.</p>
          ...
          <form onSubmit={saveAddresses} className="mt-10 space-y-6">
            ...
            <div className="rounded-2xl border border-slate-200 bg-slate-50 p-6">
              <label className="flex items-start gap-3 font-semibold"><input type="checkbox" checked={sameAsBilling} onChange={(e) => setSameAsBilling(e.target.checked)} className="mt-1 h-5 w-5" />Delivery address is the same as billing address</label>
            </div>
            {!sameAsBilling && <AddressFields title="Default Delivery Address" value={deliveryAddress} onChange={setDeliveryAddress} />}
