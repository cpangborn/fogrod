...
export async function POST(req: Request) {
  try {
    ...
    const { items, billingAddress, deliveryAddress, purchaseOrder } = await req.json();
    if (!Array.isArray(items) || items.length === 0) return NextResponse.json({ error: "Basket is empty." }, { status: 400 });

    const validAddress = (address: any) => address?.line1?.trim() && address?.city?.trim() && address?.postcode?.trim();
    if (!validAddress(billingAddress) || !validAddress(deliveryAddress)) {
      return NextResponse.json({ error: "Please provide a complete billing and delivery address." }, { status: 400 });
    }
    ...
    const orderItems = items.map((item: any) => {
      ...
      return { name: product.name, quantity, unitPrice, lineTotal: Math.round(unitPrice * quantity * 100) / 100 };
    });
    ...
    formBody.set("billing-address", addressText(billingAddress));
    formBody.set("delivery-address", addressText(deliveryAddress));
    formBody.set("items", orderItems.map((item) => `${item.name} × ${item.quantity} @ £${item.unitPrice.toFixed(2)} = £${item.lineTotal.toFixed(2)}`).join("\n"));
    ...
    const formResponse = await fetch(new URL("/__forms.html", req.url), {
      method: "POST",
      headers: { "Content-Type": "application/x-www-form-urlencoded" },
      body: formBody.toString(),
    });

    if (!formResponse.ok) throw new Error("Unable to record the trade order.");
    return NextResponse.json({ orderNumber, subtotal, vat, total });
  } catch (err) {
    console.error(err);
