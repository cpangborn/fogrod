...
export default function Navbar() {
  ...
  return (
    <>
        ...
        <div className="fixed bottom-20 right-5 z-[70] w-[calc(100vw-2.5rem)] max-w-sm rounded-2xl border border-slate-200 bg-white p-5 shadow-2xl">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="mt-1 text-sm text-slate-500">Need help? Send us a message.</p>
            </div>
            <button type="button" onClick={() => setChatOpen(false)} aria-label="Close chat" className="rounded-lg p-1 text-slate-400 hover:text-black"><X size={20} /></button>
          </div>
          <form action="https://formsubmit.co/sales@pandstankers.co.uk" method="POST" className="mt-5">
            <input type="hidden" name="_subject" value="New FOGRod Website Chat Message" />
            <input type="hidden" name="_captcha" value="false" />
            <input type="hidden" name="_template" value="table" />
            <input type="hidden" name="_next" value="https://fogrod.co.uk/" />
            <label className="mb-1 block text-sm font-semibold text-slate-700">Your name</label>
            <input name="name" required className="mb-3 w-full rounded-xl border border-slate-300 px-3 py-3 text-sm outline-none focus:border-black" />
            <label className="mb-1 block text-sm font-semibold text-slate-700">Your email</label>
            <input type="email" name="email" required className="mb-3 w-full rounded-xl border border-slate-300 px-3 py-3 text-sm outline-none focus:border-black" />
            <label className="mb-1 block text-sm font-semibold text-slate-700">Message</label>
            <textarea name="message" required rows={4} className="w-full resize-none rounded-xl border border-slate-300 px-3 py-3 text-sm outline-none focus:border-black" />
            <button type="submit" className="mt-4 flex w-full items-center justify-center gap-2 rounded-xl bg-black px-4 py-3 font-bold text-white hover:bg-slate-800">
      ...
      <button type="button" onClick={() => setChatOpen((open) => !open)} aria-label="Chat with FOGRod" className="fixed bottom-5 right-5 z-[60] flex items-center gap-2 rounded-full bg-black px-5 py-3 text-sm font-bold text-white shadow-lg transition hover:bg-slate-800">
